...
function getDateFromFormat (val, format) {
  ...
  // Extract contents of value based on format token
    if (token === 'yyyy' || token === 'yy' || token === 'y') {
      ...
      if (val.substring(iVal, iVal + 2).toLowerCase() === 'am') { ampm = 'AM' } else if (val.substring(iVal, iVal + 2).toLowerCase() === 'pm') { ampm = 'PM' } else { return 0 }
      iVal += 2
    } else {
      if (val.substring(iVal, iVal + token.length) !== token) { return 0 } else { iVal += token.length }
    }
  }
